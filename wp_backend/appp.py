...
# Save clues and words to a file
def save_clues(clues, words):
    try:
        ...
        print(f"Error saving clues and words: {e}")

# Load initial data
default_clues, default_words = load_clues()
scores = {"user1": 0, "user2": 0, "user3": 0, "user4": 0, "user5": 0}

# Function to generate a shuffled grid
def generate_shuffled_grid(words, size=10):
    ...
    # Fill remaining empty spaces with random alphabets
...
def get_puzzle():
    grid = generate_shuffled_grid(default_words)
...
def add_clue_word():
    ...
    # Update global lists and save to file
    new_entry = {"clue": clue, "word": word.upper()}
    default_clues.append(new_entry)
    default_words.append(word.upper())
...
if __name__ == '__main__':
    print("Starting the Word Puzzle API server...")
    print("Using clues file:", CLUES_FILE)
